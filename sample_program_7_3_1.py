...
if add_nonlinear_terms_flag:
    x_tmp = sample_functions.add_nonlinear_terms(x_tmp)  # 説明変数の二乗項や交差項を追加
    x = x_tmp.drop(x_tmp.columns[x_tmp.std() == 0], axis=1)  # 標準偏差が 0 の説明変数を削除
else:
    x = x_tmp.copy()

# 時間遅れ変数の追加
dataset_with_dynamics = sample_functions.add_time_delayed_variable(pd.concat([y, x], axis=1), dynamics_max, dynamics_span)
